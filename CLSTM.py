...
class MyCLSTM(nn.Module):
    # Constructor
    def __init__(self, input_channels=64, hidden_channels=[64],
                 kernel_size=5, bias=True):
        super(MyCLSTM, self).__init__()
    ...
    def forward(self, input):
        ...
        for layer in range(self.num_layers):
            # do forward
            name = 'cell{}'.format(layer)
            (h, c) = self.internal_state[layer]

            input, c = getattr(self, name)(
                input, h, c)  # forward propogation call
            self.internal_state[layer] = (input, c)
...
class BDCLSTM(nn.Module):
    ...
    def __init__(self, input_channels=64, hidden_channels=[64],
        ...
        # self.conv = nn.Conv2d(
        #     hidden_channels[-1], 1, kernel_size=1)
        # self.soft = nn.Softmax2d()

    def init_hidden(self, x):
        self.forward_net.init_hidden(x)

    def forward(self, input):
        y = self.forward_net(input)
        # xforward = torch.cat((x1, x2), dim=1)
        # xreverse = torch.cat((x3, x2), dim=1)
        # inputreverse = torch.flip(input, dims=[1]) #batch, steps,...

        # yreverse = self.reverse_net(inputreverse)
